...
function flexipages_init()
{
	...
	function flexipages_list($page_array, $level = 0, $args='')
	{
		if(!$page_array)
			return;
		...
		if (!isset($list_item_tag) || $list_item_tag == '')
		  $list_tag = 'li';
		...
		foreach($page_array as $page) {
			if($page['ID'] == 'home') {
				...
				$classes = get_post_meta($page['ID'], 'class');
				if ($classes != '')
				  if (is_array($classes))
			...
			$pagelist .= str_repeat("\t", $level+1).'<' . $list_item_tag . ' class="'.$class.'"><a href="'.$page['link'].'" title="'.$page['title'].'">' . preg_replace('/^(.*)$/', $title_format, $page['title']) . '</a>'.$date;
			if($page['children'])
				$pagelist .= flexipages_list($page['children'], $level+1, $args);
			$pagelist.= "</$list_item_tag>\n";
		}
		if($pagelist)
			$pagelist = str_repeat("\t", $level).($list_tag?"<$list_tag>\n":'')."{$pagelist}".str_repeat("\t", $level).($list_tag?"</$list_tag>":'');
		return $pagelist;
	}
...
//		echo "<pre>"; print_r($page_array); echo "</pre>";
		...
		if($dropdown == 'on' || $dropdown == 1) {
			...
			$pages = flexipages_list($page_array, 0, "list_tag={$list_tag}&list_item_tag={$list_item_tag}&title_format={$title_format}");
...
//		echo "<pre>"; print_r ($options[$number]); echo "</pre>";
		...
		if($pagelist = flexipages("echo=0&sort_column={$sort_column}&sort_order={$sort_order}&exclude={$exclude}&include={$include}&show_subpages={$show_subpages}&hierarchy={$hierarchy}&depth={$depth}&show_home={$show_home}&show_date={$show_date}&date_format={$date_format}&dropdown={$dropdown}&list_tag={$list_tag}&list_item_tag={$list_item_tag}&title_format={$title_format}")){
	...
	function flexipages_widget_control($widget_args)
	{
		...
		if ( !$updated && !empty($_POST['sidebar']) ) {
			...
			foreach ( (array) $_POST['flexipages_widget'] as $widget_number => $flexipages_widget ) {
				...
				$list_tag = strip_tags(stripslashes($flexipages_widget['list_tag']));
				$list_item_tag = strip_tags(stripslashes($flexipages_widget['list_item_tag']));
				$title_format = stripslashes($flexipages_widget['title_format']);
				
				$options[$widget_number] = compact('title', 'sort_column', 'sort_order', 'exinclude', 'exinclude_values', 'show_subpages_check', 'show_subpages', 'hierarchy', 'depth', 'show_home_check', 'show_home', 'show_date', 'date_format', 'dropdown', 'list_tag', 'list_item_tag', 'title_format');
			}
		...
		$list_tag = attribute_escape($options[$number]['list_tag']);
		$list_item_tag = attribute_escape($options[$number]['list_item_tag']);
		$title_format = attribute_escape($options[$number]['title_format']);
		...
		<table cellpadding="10px" cellspacing="10px">
			...
			<tr>
				<td><label for="flexipages-list-tag-<?php echo $number; ?>"><?php _e('List tag', 'flexipages'); ?></label></td>
				<td><input class="widefat" id="flexipages-list-tag-<?php echo $number; ?>" name="flexipages_widget[<?php echo $number; ?>][list_tag]" type="text" value="<?php echo $list_tag; ?>" /></td>
			</tr>
			<tr>
				<td><label for="flexipages-list-item-tag-<?php echo $number; ?>"><?php _e('List item tag', 'flexipages'); ?></label></td>
				<td><input class="widefat" id="flexipages-list-item-tag-<?php echo $number; ?>" name="flexipages_widget[<?php echo $number; ?>][list_item_tag]" type="text" value="<?php echo $list_item_tag; ?>" /></td>
			</tr>
			<tr>
				<td><label for="flexipages-title-format-<?php echo $number; ?>"><?php _e('Title format', 'flexipages'); ?></label></td>
				<td><input class="widefat" id="flexipages-title-format-<?php echo $number; ?>" name="flexipages_widget[<?php echo $number; ?>][title_format]" type="text" value="<?php echo $title_format; ?>" /></td>
			</tr>
		</table>
			<p>	
