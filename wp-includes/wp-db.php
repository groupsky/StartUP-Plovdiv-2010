...
'/*/WP_I18N_DB_CONN_ERROR*/, $dbhost), 'db_connect_fail');
	...
	function set_prefix($prefix) {
		...
		if ( preg_match('|[^a-z0-9_]|i', $prefix) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Невалидна представка за името на базата от данни!'/*/WP_I18N_DB_BAD_PREFIX*/);
...
<h1>Грешка при отваряне на базата от данни</h1>
<p> Беше установена връзка със сървъра (което означава, че потребителското име и паролата са верни) но базата от данни <code>%1$s</code> не бе отворена успешно.</p>
<ul>
<li>Съществува ли такава база от данни?</li>
<li>Има ли право потребителят <code>%2$s</code> да използва базата от данни <code>%1$s</code>?</li>
<li>На някои сървъри пред името на базата се поставя потребителското ви име. Дали името й не е нещо като <code>%2$s_%1$s</code>?</li>
</ul>
<p>Ако не можете да настроите базата си от данни, <strong>свържете се с вашия хостинг</strong>. Ако не намерите помощ на друго място, опитайте във <a href=\'http://bg.forums.wordpress.org/\'>форумите за поддръжка на WordPress</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser), 'db_select_fail');
			return;
		}
	}
	...
	function print_error($str = '') {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR_FULL*/'Грешка: %1$s при заявка %2$s изпълнена от %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller);
		else
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR*/'Грешка: %1$s при заявка %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query);
	...
	function get_row($query = null, $output = OBJECT, $y = 0) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Типът на резултата(output_type) трябва да е един от следните: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
